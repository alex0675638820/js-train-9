...
function findElementGreaterThan(arr, num) {
  ...
  Array.isArray(arr)? arr : 'undefined';	
  // повертаємо undefined
  // Використовуємо метод find для пошуку першого елементу, який є більшим за задане число num.
	const maxelem = arr.find((elem, index, array) => {
		if(elem > num) {
			console.log(elem);
		}
	})
  // Перевіряємо чи є елемент який задовільняє умову
//   const maxelem = arr.find(({elem}) => elem > num ? console.log(maxelem) : 'Числа яке задовільняе умову не знайдено');
...
function findIndexByProperty(arr, name) {
  ...
  const i = arr.findIndex(({name}) => name === j);
  return i
}
...
function findLastEvenNumber(arr) {
  ...
  Array.isArray(arr)? arr : 'Вхідний параметр має бути масивом';
  // За допомогою метода findLast знаходимо останнє парне число в масиві.
  const i = arr.findLast(({elem}) => elem % 2 !== 0);
...
function getPositiveArrayValues(arr) {
  ...
  // Конвертуємо ітератор в масив.
	const j = [...result].filter((elem, index, result) => elem > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const i = arr.splice(startIdx);
  // За допомогою методу map() подвоюємо кожен елемент масиву.
  const j = [...arr].map((elem) => elem * 2);
...
function sumAndPrint(arr) {
  ...
  const a = arr.reduce((num, elem) => num + elem, 0);
  // За допомогою методу forEach() виводимо кожен елемент масиву в консоль.
  arr.forEach((elem, index, arr) => {console.log(elem)});
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  // За допомогою методу reduceRight() обчислюємо суму останніх n елементів масиву.
	const j = a.reduceRight((num, elem, elemIndex, a) =>
	++num , n);
  // За допомогою методу forEach() виводимо кожен елемент з останніх n в консоль.
	const i = a.forEach(element => console.log(element));
...
const i = arr.splice(index, 0, element);
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((num, elem, elemIndex, arr) =>{
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((elem, index, arr) => {index[0] === index[1]  
	return elem});	
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((num, elem, elemIndex, arr) =>{
	return num + elem.price;
	}
  , 0);
